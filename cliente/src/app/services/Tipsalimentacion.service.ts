...
@Injectable()
export class TipsalimentacionService{
public url:string;
...
return this._http.post(this.url+'tips',params,{headers:headers});
}
...
let headers=new HttpHeaders().set('Content-Type','application/json')
							 .set('Authorization',token);
return this._http.get(this.url+'tips/'+page,{headers:headers});
}
...
getPublicationsUserm(token,user_id,page=1):Observable<any>{

let headers=new HttpHeaders().set('Content-Type','application/json')
							 .set('Authorization',token);
return this._http.get(this.url+'tips-userm/'+user_id+'/'+page,{headers:headers});
}
...
let headers=new HttpHeaders().set('Content-Type','application/json')
							 .set('Authorization',token);
return this._http.delete(this.url+'tips/'+id,{headers:headers});
}
