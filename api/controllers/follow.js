...
//LISTAR DE MANERA CLARO LOS QUE SIGO PARA LOS MENSAJES
...
function getmyuserentrenador(req,res){

User.find({'role':'ROLE_TRAINER'}).exec((err,follows)=>{
  if(err) return res.status(500).send({message: 'Error en el servidor'});
...
User.find({'role':'ROLE_USER'}).exec((err,follows)=>{
  if(err) return res.status(500).send({message: 'Error en el servidor'});
...
module.exports={saveFollow,deleteFollow,getFollowUsers,getFollowedUsers,getMyFollow,getmyuserentrenador,getmyusercliente
}
