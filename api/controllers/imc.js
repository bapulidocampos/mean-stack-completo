...
imc.imc=params.imc;
imc.mbasal=params.mbasal;
imc.tmb=params.tmb;
imc.actividad=params.actividad;
imc.file='null';
imc.filev='null';
imc.user=req.user.sub;
//imc.vector=[100,200,300]; 
//console.log(imc.vector);
//console.log(imc.vector[0]);

imc.created_at=params.created_at;
imc.created_at2=moment().unix();
imc.created_at=params.created_at;
...
//traemos solo los pesos
/*
Imc.find({user:req.user.sub}).populate('peso').exec((err,pesos)=>{
...
Imc.find({user:req.user.sub}).populate('').exec((err,followsse)=>{
if(err)return res.status(500).send({message:'error al devolver el seguimiento'});

var follow_cleans=[];
var follow_cleanss=[];
var follow_cleansss=[];
var follow_cleanssss=[];
//var hola=[];
//hola[0]='11';
//hola[1]='12';
...
//lleno vector peso
followsse.forEach((followsss)=>{
	//console.log("nuevo "+followsss);
	//para añadir el id pero estoy trayendo completo el objeto
follow_cleans.push(followsss.peso);
console.log("nuevo vector peso "+follow_cleans);
});
//lleno vector fecha
followsse.forEach((followsss)=>{
	//console.log("nuevo "+followsss);
	//para añadir el id pero estoy trayendo completo el objeto
follow_cleanss.push(followsss.created_at);
console.log("nuevo vector fecha "+follow_cleanss);
});

//lleno el vector de tasametabolica basal
followsse.forEach((followsss)=>{
	//console.log("nuevo "+followsss);
	//para añadir el id pero estoy trayendo completo el objeto
follow_cleansss.push(followsss.tmb);
console.log("nuevo vector tmb "+follow_cleansss);
});

//lleno el vector de imc 
followsse.forEach((followsss)=>{
	//console.log("nuevo "+followsss);
	//para añadir el id pero estoy trayendo completo el objeto
follow_cleanssss.push(followsss.imc);
console.log("nuevo vector imc "+follow_cleanssss);
});
...
Imc.find({user:{"$in":follow_clean}}).sort('-created_at2').populate('user ').
...
function getPublicationsvisitado(req,res){
var page=1;
console.log("por aqui traje el usuario visitado");
...
Follow.find({user:req.params.user}).populate('followed').exec((err,follows)=>{
if(err)return res.status(500).send({message:'error al devolver el seguimiento'});

var follow_clean=[];

Imc.find({user:req.params.user}).populate('').exec((err,followsse)=>{
if(err)return res.status(500).send({message:'error al devolver el seguimiento'});
