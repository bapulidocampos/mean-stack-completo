...
Exercise.find({}).sort('-created_at').populate('user').
...
//var file_ext=ext_split[1];
//console.log(publications[0]);
Exercise.find({category:'bicep'}).sort('-created_at').populate('user').
paginate(page,itemsPerPag,(err,exercises,total)=>{
if(err)return res.status(500).send({message:'error al devolver publicaciones'});
if(!exercises)return res.status(404).send({message:'No hay publicaciones'});
...
function getPublicationssscardio(req,res){
	var hola=totale(req,res);
	console.log("que paso"+hola);
var page=1;
if(req.params.page){
...
//var file_ext=ext_split[1];
//console.log(publications[0]);
Exercise.find({category:'cardio'}).sort('-created_at').populate('user').
paginate(page,itemsPerPag,(err,exercises,total)=>{
if(err)return res.status(500).send({message:'error al devolver publicaciones'});
if(!exercises)return res.status(404).send({message:'No hay publicaciones'});
...
function getPublicationssstricep(req,res){
	var hola=totale(req,res);
	console.log("que paso"+hola);
var page=1;
if(req.params.page){
...
//var file_ext=ext_split[1];
//console.log(publications[0]);
Exercise.find({category:'tricep'}).sort('-created_at').populate('user').
paginate(page,itemsPerPag,(err,exercises,total)=>{
if(err)return res.status(500).send({message:'error al devolver publicaciones'});
if(!exercises)return res.status(404).send({message:'No hay publicaciones'});
...
//--------  obtengo todos los clientes   ROLE_USER      ----------------
function getmyusercliente(req,res){

Exercise.find({}).exec((err,follows)=>{
...
module.exports={
	...
	getPublicationss,
	getPublicationsss,
	getPublicationssstricep,
	getPublicationssscardio,
	getmyusercliente
}