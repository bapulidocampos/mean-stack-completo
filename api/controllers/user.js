...
function saveUser(req,res){
	...
	var user=new User();
	if(params.name && params.surname && params.nick && params.email && params.password && params.role
		&& params.date_birth&& params.sexo)
	{
...
var recaptcha=params.recaptcha;
console.log("aqui estoy en login api");
console.log(recaptcha);
 const secretKey = '6LeVxtUaAAAAAJR3X9K0golpJ3mUTe0nrjqxG-_q';
...
if(!recaptcha){
	console.log("no envio nada en el backend");
	return res.json({ success: false, msg: 'Please select captcha' });
}


//const veri='https://www.google.com/recaptcha/api/siteverify?secret=secretKey&response=recaptcha';
const veri='https://www.google.com/recaptcha/api/siteverify?secret=6LeVxtUaAAAAAJR3X9K0golpJ3mUTe0nrjqxG-_q&response='+recaptcha;
console.log(veri);
...
var nombre = async function (verifyURL){
 //   const url = `https://api.github.com/users/${username}`
    var respuesta = await fetch (veri)
    var response = await respuesta.json();
    guardarstatus.push(response);
   
   if(guardarstatus[0].success !== undefined && !guardarstatus[0].success) {
            //return res.send({ "message": "Captcha validation failed" });
            
           return mensaje="Captcha invalidation";
        }
   else{
   console.log("a 1 "+guardarstatus[0].success);
  mensaje="Captcha validation";

   // console.log("a 2 "+guardarstatus[1].success);
   // return  res.send({ "message": "Captcha validation " });
    return mensaje
   }
 

 // return guardarstatus; 
...
console.log(mensaje);
//nombre(verifyURL);
nombre(verifyURL).then(val => console.log(val));
console.log("aqui voy ");
console.log(mensaje);
//console.log("vector completo"+val);
for (var i = 0; i < guardarstatus.length; i++) {
	console.log("vector completo");
	console.log("vector completo "+i+" "+guardarstatus[i]);
}
			...
			bcrypt.compare(password,user.password,(err,check)=>{
				if(check){
					...
					if(params.gettoken){
						...
						user.password=undefined;//devolver los datos de los usuario en claro
					return res.status(200).send({user});//si check es correcto .devolver datos de usuarios
...
function updateUser(req,res){
	...
	//si user id es ! usuario identificado no tiene permiso para actualizar
